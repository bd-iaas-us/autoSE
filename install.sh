...
if [[ $# = 0 ]]; then
    AILINT_BINARY_URL=$github_repo/releases/latest/download/ailint-$target.zip
else
    AILINT_BINARY_URL=$github_repo/releases/download/$1/ailint-$target.zip
fi
...
curl -L -o "${TMPDIR}/${BINARY_NAME}.zip" "${AILINT_BINARY_URL}"
...
mv "${TMPDIR}/dist/ailint-$target" "${TARGET_DIR}/${BINARY_NAME}"
