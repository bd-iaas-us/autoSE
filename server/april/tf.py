...
def handle_tf(request) -> str:
    ...
    for resource in resp1_json['resources']:
        ...
        if 'type' in resource:
            resource_file = '_'.join(resource['type'].split('_')[1:]) + '.html.markdown'
        if 'resource_type' in resource:
            resource_file = '_'.join(resource['resource_type'].split('_')[1:]) + '.html.markdown'
    ...
    hcl_code_str = resp2[resp2.find(start)+len(start):resp2.rfind(end)].strip()
