...
import yaml
from index import QueryLint, openai_template, templates
...
def main_layout():
    ...
    new_text = st.text_area("Input Text",
                            value=prompt,
                            height=500,
                            help="KEEP the context prompt, DO NOT delete!!")
    ...
    if st.button("LINT THIS FILE"):
        if st.session_state.file_path:
            ...
            thread = threading.Thread(target=worker,
                                      args=(file_content, result_queue))
            thread.start()
